/* uLisp SAM/SAMD Version 1.9a - www.ulisp.com
   David Johnson-Davies - www.technoblogy.com - 9th August 2017
...
#define BUFFERSIZE 34  // Number of bits+2
  ...
  uint8_t _end;
#else
  #define WORKSPACESIZE 4096              /* Cells (8*bytes) */
  #define SYMBOLTABLESIZE 512             /* Bytes */
  object Workspace[WORKSPACESIZE] WORDALIGNED;
  #define SDCARD_SS_PIN 10
  uint8_t _end;
...
char *MakeFilename (object *arg) {
  ...
  if (stringp(arg)) {
    ...
    strcpy(&buffer[i], ".IMG");
  } else strcpy(buffer, "ULISP.IMG");
  return buffer;
}

int saveimage (object *arg) {
  SD.begin(SDCARD_SS_PIN);
  File file = SD.open(MakeFilename(arg), FILE_WRITE);
  if (!file) error(PSTR("Problem saving to SD card"));
  if (stringp(arg)) arg = NULL;
  unsigned int imagesize = compactimage(&arg);
  file.seek(0); // Overwrite previous saved image
...
int loadimage (object *filename) {
  SD.begin(SDCARD_SS_PIN);
  File file = SD.open(MakeFilename(filename));
  if (!file) error(PSTR("Problem loading from SD card"));
  object *arg = SDReadPtr(file);
...
object *read (gfun_t gfun) {
  object *item = nextitem(gfun);
  if (item == (object *)KET) error(PSTR("Unexpected end of list"));
...
void setup() {
  ...
  initworkspace();
  initenv();
  pfstring(PSTR("uLisp 1.9a "), pserial); pln(pserial);
}
